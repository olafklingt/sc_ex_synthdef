defmodule UGen.Maker do
  ...
  def make_func_file(name, methods) do
    path = Path.dirname(__ENV__.file)
  ...
  def func_args_list(arguments) do
    for x <- arguments do
      n = Enum.at(x, 0)
      v = Enum.at(x, 1)
  ...
  def args_list(arguments) do
    for x <- arguments do
      String.to_atom(Enum.at(x, 0))
    end
  end

  def fields_list(arguments) do
    for x <- arguments do
      n = Enum.at(x, 0)
      v = Enum.at(x, 1)
  ...
  def make_def_file(name, handle, rate, number_of_outputs, arguments) do
    path = Path.dirname(__ENV__.file)
